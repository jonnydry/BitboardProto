...
export const ReactionDisplay = memo<ReactionDisplayProps>(
  ({ counts, userReaction, onReact, disabled = false, compact = false }) => {
    ...
    return (
      <div className="flex flex-wrap items-center gap-1">
        ...
        {activeReactions.map((emoji) => (
          ...
          >
            <span className="flex items-center justify-center text-[1.05rem] leading-none">
              {emoji}
            </span>
...
const ReactionPickerPopup = memo<ReactionPickerPopupProps>(
  ({ onSelect, userReaction, onClose: _onClose }) => {
    return (
      <div
...
export const ReactionBar = memo<ReactionBarProps>(
  ({ eventId, nostrEventId, disabled = false, compact = false }) => {
