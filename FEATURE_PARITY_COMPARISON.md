# Feature Parity Comparison: App.tsx vs AppNew.tsx (HISTORICAL / ARCHIVE)

> Note: AppNew.tsx migration was superseded. Current production code uses App.tsx + features/layout/\* split (AppContext, handlers, Sidebar, AppHeader etc). AppNew no longer exists in source. This doc preserved for history. See docs/architecture.md and README for current state (rich Nostr client with bits+geo focus).

## Executive Summary

**App.tsx** (current) + features/layout is the **complete, production-ready** implementation.
**AppNew.tsx** (216 lines, removed) was an **incomplete migration** to a context-based architecture.

**Status (historical)**: AppNew.tsx was missing **11 critical features** and had **5 implementation differences**.
